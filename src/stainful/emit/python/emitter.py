...
class _Emitter:
    ...
    def _sub_accessors(self, r: Resource, *, is_async: bool) -> str:
        ...
        return "\n\n".join(
            f"    @cached_property\n"
            f"    def {snake(s.name)}(self) -> {pre}{pascal(s.name)}:\n"
            f"        return {pre}{pascal(s.name)}(self._client)"
    ...
    def _resource_src(self, r: Resource) -> str:
        ...
        sub_import = "".join(
            f"from .{snake(s.name)} import Async{pascal(s.name)}, {pascal(s.name)}\n"
            for s in r.subresources
        )
...
'''
    ...
    def _client_src(self) -> str:
        ...
        def body(is_async: bool) -> str:
            ...
            attrs = "\n".join(
                f"    {snake(r.name)}: resources.{pre}{pascal(r.name)}"
                for r in resources
            )
            inits = "\n".join(
                f"        self.{snake(r.name)} = resources.{pre}{pascal(r.name)}(self)"
                for r in resources
            )
...
'''
    ...
    def _pkg_init_src(self) -> str:
        ...
        return (
            ...
            + f"__all__ = {['%s' % self.brand, 'Async%s' % self.brand, 'NotGiven', 'not_given', 'NOT_GIVEN', 'Omit', 'omit', *errs]!r}\n"
        )
    ...
    def emit(self) -> None:
        ...
        (self.root / "resources" / "__init__.py").write_text(
            ...
            + "".join(
                ...
                f"Async{pascal(r.name)}, {pascal(r.name)}\n"
